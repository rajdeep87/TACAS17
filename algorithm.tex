...
\end{definition}
%
The algorithm begins with an empty propagation trail $\trail$, 
a empty reason trail $\reasons$ and an abstract value $\top$.
...
trail $\trail$.  The decision $\decisionvar$ refines  the current abstract value $\abs(\trail)$ represented by the trail, i.e.\ $\abs(\trail\cdot\decisionvar)\sqsubseteq \abs(\trail)$.
