...
The input to the algorithm is a program in the form of the set of its abstract transformers $\abstransset$ w.r.t.\ an abstract domain~$\domain$. The algorithm is parametrised by propagation, decision, and conflict analysis
heuristics.  
...
propagation trail to transformers $\abstrans\in\abstransset$ that were used to
...
ACDCL heuristically chooses one reason and learns it by adding at as
an abstract transformer to $\abstransset$. Then it backtracks by
removing the tail of the $\trail$ up to a point where it becomes
consistent, and continues making deductions from there.
If $\analyzeconflict$ returns $\false$ then $\formula$ is unsatisfiable
and we return  \textsf{safe}.
