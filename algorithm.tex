...
$\widehat{ded_{\varphi}}$ \pscmt{???} for a formula $\varphi$ over an abstract 
...
fixed point over the transformers in F \pscmt{what is F?} that refines the abstract valuation 
$v$, similar to the BCP \pscmt{mentioned before?} step in SAT solvers.  If the result of $deduce$ 
...
For example, a decision in interval domain restricts the range of 
...
{\em dual widening} \pscmt{what do we learn?}.  The procedure $deduce$ is called next to infer new 
...
algorithm returns the error trace $v$ \pscmt{this is not a trace} and terminates with UNSAFE or the 
