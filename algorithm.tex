...
\pscmt{map?} $\mathcal{R}$.  The propagation trail is an initially
...
\pscmt{certain?} elements of the propagation trail to transformers
...
consistent with the state of the solver. \pscmt{what happens if we cannot make any decision any more? -- then we should either have deduced $\bot$ or $\mathit{abs}(\mathcal{T})$ must be $\gamma$-complete} If $d$ refines the current abstract 
