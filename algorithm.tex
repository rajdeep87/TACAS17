...
\lIf{$\mathit{result}$ = \sat} {
  ...
  $d \leftarrow \decide_{\decheur}(\abs(\trail))$ \;
  $\trail \leftarrow \trail \cdot d$ \; 
...
trail $\trail$ and a reason trail \pscmt{map?} $\reasons$.
...
\pscmt{define trail refinement}
\pscmt{define concatenation operator $\cdot$}
\pscmt{explain $\top$ assignment to $\reasons$}
...
and makes a new decision by a call to $\decide$ which returns
...
solver. \pscmt{what happens if we cannot make any decision any more?
  ...
  $\abs(\trail)$ must be $\gamma$-complete} If $d$
refines the current abstract value, the corresponding propagation
and reason trail, $\trail$ and $\reasons$ are updated.
%
...
$\analyzeconflict$ phase to learn the reason for the
...
abstract value does not abstractly satisfy the formula
\pscmt{define}.  If no further backtrack is possible, then the
algorithm terminates and $\formula$ is \textsf{safe}. Else, the
