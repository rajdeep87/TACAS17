\section{Program Model and Abstract Domain}
...
properties given as assertions $\mathcal{A}$ in the program.
%
...
where $\mathcal{P}$ are the statements of the transformed program in the
...
%
Based on the above program representation, we define a safety equation \pscmt{why ``equation''?} as, 
$\varphi:= \bigwedge_{s\in\mathcal{S}} s$. The formula $\varphi$ is 
...
In this paper we use a product domain $\mathcal{B}^b\times\mathcal{R}$ where
...
numerical (bitvector) variables of respective
...
written as the vector $(g_0,g_1,c,g_2)$; then we will denote an abstract value $(\top, \false, \true, \top) \in \mathcal{B}^4$ as $\neg g_1 \wedge c$.

%-------------------------------------------------------------------------------
\paragraph{Template Polyhedra Abstract Domain}
%
An abstract value of the template polyhedra domain~\cite{sriram}
represents a set $X$ of values of numerical variables $\vec{x}$. For
example, in the program in Figure~\ref{ssa}, we have four numerical
variables, written as the vector $\vec{x}:=(x_0,x_1,x_2,z)$.  We can
represent sets of values for $\vec{x}$ such that
...
\caption{Example of an octagon \pscmt{BUG: should be $x\geq -2$, $y\geq -1$; please rename x to $x_1$ and $y$ to $z$}}\label{octagon}
\vspace*{-3ex}
\end{wrapfigure} 
...
\[\llbracket s
\rrbracket^\sharp(a)=a\sqcap \alpha(\{c\mid c\models s\})\] where we write 
$c\models s$ if the concrete value $c$ satisfies the constraint $s$.
%$\forall \vec{y}\in c: (s[\vec{y}/\vec{x}]
...
%
For example, we can deduce $\llbracket
x=2(y+z)\rrbracket^\sharp(a)=a\sqcap(-2\leq x\leq 6)$ for the abstract value $a=(0\leq y \leq 2 \wedge 1 \leq y-z \leq 1)$.
%
...
$\varphi$ if for every variable $x$ in $\varphi$, there is a concrete solution 
