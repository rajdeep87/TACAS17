...
bounded unwindings of programs with loops or recursion\rmcmt{why can't we simply 
say we consider programs with bounded loops}.  We assume
that safety properties are given as assertions $\mathcal{A}$ in the program.
%
Such a program is represented as set
$\mathcal{S}=\mathcal{P}\cup\{\neg \bigwedge_{a\in\mathcal{A}} a\}$
where $\mathcal{P}$ are the statements of the program in the form of
...
%-------------------------------------------------------------------------------
SAT solvers operates on the partial assignment domains~\cite{sas12,dhk2013-popl}.  
We present an instantiation of ACDCL as program analyzer that uses relational
domains.  
% 
...
e.g.\ for the boolean variable vector $(g_1,c)$ \rmcmt{what is g1, what is c?} 
with abstract value $(\false,\top)$ we will write $\neg g_1$.

%-------------------------------------------------------------------------------
\paragraph{Template Polyhedra Abstract Domain}
%
The template polyhedra domain~\cite{sriram} represents sets $X$ \pscmt{split this up into two sentences} of
valuations of a vector of variables $\vec{x}$ \rmcmt{not clear what is a
valuation?} such that
...
operations, such as $\sqcap$, $\sqcup$, etc~\cite{sriram,GS07b,BJKS15}\rmcmt{like what operations?}. In our implementation we use
...
of the template polyhedra domain. $\true$ is $\top$ and $\false$ is $\bot$.
\pscmt{give $\mat{C}$ and $\vec{d}$ for this example}
...
\rmcmt{can we rephrase this statement to say that -- In this paper, we used
interval, octagon and equality ?}
...
The best abstract transformer is \rmcmt{is this equation updated ?}
\[\llbracket s
\rrbracket^\sharp(a)=\bigsqcap \{a'\mid a'\sqsubseteq a, a'\models s\}\] where we write \pscmt{fix this}
