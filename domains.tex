...
\begin{definition}{(Meet Irreducible)} A \emph{meet irreducible} $m$ 
...
The meet irreducibles in the Boolean domain $\mathcal{B}$ are $\true$
and $\false$. The meet irreducibles in the interval domain are all
intervals that can be represented by a single inequality, i.e.\ $x \leq d$ or $x \geq d$ for constants $d$.

\begin{definition}{(Complementable Meet Irreducible)} 
A meet irreducible $m\in A$ is called complementable iff $\exists
...
The meet irreducibles in the Boolean and template polyhedra domains
are all complementable.
