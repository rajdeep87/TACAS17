...
The meet irreducibles in the partial Boolean assignments domain and
...
\[\llbracket s
\rrbracket^\sharp(a)=\min \{a'\mid a'\models s\}\] 
where we write
...
of $a$. Any abstract transformer that computes a non-minimal element
is still a sound over-approximating transformer and can be used in our
algorithm.
...
%$a=(x-y \leq 1, y-x \leq 1)$
%$\llbracket x=y+z\rrbracket^\sharp($
...
$\forall m_i \in M, \meet(m_i) = a$, where $max(i) = |M|$.
\end{definition}
For example, the meet decomposition of an interval is given as follows.
...
of the search space and allows case-based analysis.  
%
\begin{definition}{(Precise complement)} A precise complement of an abstract
element $a \in A$ in abstract lattice $A$ is an element $\bar{a}$ such that 
$\gamma(\bar{a} = \neg \gamma(a))$.
\end{definition}
%
...
For example, intervals lack complements, but the meet decomposition 
of intervals into meet-irreducibles have precise complements.  
Numerical abstract domains that admit complementable decomposition 
are shown in Figure~\ref{complement}.  Our template-based domain 
implementation precisely complements a meet irreducible in an abstraction.
% 
\begin{table}
\begin{center}
{
...
Standard abstract interpretation do not require complementation property
The application of set-complements on over-approximating 
domains does not give necessary invariants required to reason about assertions
in programs. 
Abstract Domain library, such as APRON C library~\cite{apron} is specialized 
