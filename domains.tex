...
In this paper we use a product domain $\domain=\booldomain^\booldim\times\reldomain$ where
$\booldomain$ is the boolean domain
$\langle\{\true,\false,\bot,\top\},\Rightarrow,\wedge,\vee\rangle$, 
$\booldim$ is the number of boolean variables in the program, 
...
$\booldomain^\booldim$ by their concretisations to propositional formulae,
...
represents a set $\numconcval$ of values of numerical variables $\vec{\numvar}$. For
...
\[\abstrans{\domain}{\constraint}(\absval)=\absval\meet \alpha(\{\val\mid \val\models \constraint\})\] where we write 
