...
\hline
 SSA & Octagon \\
\hline
\begin{minipage}{4.20cm}
$\begin{array}{rcl}
(g_0 = \true) \wedge (c = 0 \leq v\leq N) \\
...
$\begin{array}{l@{\,\,}c@{\,\,}l}
C &\iff& ((x \geq -5) \land (x \leq 5) \land \\
  &    & (z \geq 0) \land (z \leq 25) \land \\
  &    & (x-z \leq 5) \land (x-z \geq -30) \land \\
  &    & (x+z \geq -5) \land (x+z \leq 30))  
\end{array}$
\end{minipage}
\\
\hline
\end{tabular}
\caption{SSA and Octagonal Inequalities}
\label{ssa}
\end{figure}
%
...
x=2(y+z)\rrbracket^\sharp(a)=(-2\leq x\leq 6)$ for $a=(0\leq y \leq 2 \wedge 1 \leq y-z \leq
1)$.
%
...
element of the partial assignments domain is decomposable into
...
abstract model search and learning in abstractions.  We formalise 
this in the sequel.
%
\begin{definition} 
...
\end{definition}
%
The meet irreducibles in boolean domain $\mathcal{B}$ are 
$\{x \mapsto \true\}$ and $\{x \mapsto \false\}$. The meet 
