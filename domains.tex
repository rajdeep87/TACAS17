...
In this section, we define a program and the notion of concrete domain 
and abstract domain. 
\subsection{Programs:}
%
...
\begin{definition}{(Meet Irreducibles)} A meet irreducible is a minimum complementable 
element that cannot be expressed in terms of the meet over a set of other 
elements. Formally, a {\em meet irreducible} $M$ in a complete lattice 
structure $A$ is an element $M \in A$ that has the following property.
...
complementation operator.  For example, the octagon in 
Figure~\ref{octagon} can be written as a conjunction of:
