...
non-singleton element of the partial assignment domain is decomposable into
...
\rmcmt{Convention to be followed: In SAT world, partial assignment (PA) is a data structure. Previous ACDL paper 
showed that PA is an abstract domain and called it partial assignment domain. Likewise, in this paper, 
whenever we refer to SAT, we also call partial assignment domain. But When we refer 
to our ACDCL product domain, we call boolean domain and polyhedra domain. We follow this 
convention throughout. This would make a SAT reader happy as well as a Abstract Interpreter not sad !!}
%
...
\rmcmt{$\gamma$ is not defined before and used here for the first time}
