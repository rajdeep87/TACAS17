...
\rmcmt{1.5X} more efficient compared to propositional SAT in terms of number of decisions, propagations 
and learning \pscmt{what learning???}.    
%
\begin{table}
\begin{center}
{
...
Figure~\ref{prop-dec} presents a comparison between {\em forward} analysis \pscmt{iteration/propagation strategy} and
{\em chaotic} analysis in ACDCL \pscmt{what about backward?}.  The choice of analysis influences the 
...
can be visualised in Figure~\ref{prop-dec}. \pscmt{Are the graphs up-to-date?} Unlike the forward analysis, 
...
significantly reduces the total number of decisions and learning. \pscmt{Why?} 
\Omit {
...
a DPLL-style conflict analysis without learning \pscmt{you learn the negation of the last decision}.  The effect of UIP computation 
...
which require relational domains. \pscmt{Is there a significant difference between intervals and octagons that indicates that for relational domains DPLL is even worse than in the propositional case?}        
