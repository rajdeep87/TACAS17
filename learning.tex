...
assignment that is sufficient to infer $c$ \pscmt{???}.  Thus, a conflict 
...
set of structures that do not satisfy a formula \pscmt{this explanation does not help the reader, who will mix up counterexamples and countermodels}.  Since relational 
...
that involve the same variables.  The first-UIP is a node closest \pscmt{unique?} to the 
...
conflict node and iteratively finds new nodes that contradict \pscmt{we have to define what that means in the preliminaries} with the 
...
Choosing cut0 yields a learnt clause \pscmt{use $\leq$ instead of $<$} 
...
\pscmt{push constants to the lhs of the inequalities; labels cut0, cut1 not fully visible}
}
\end{figure} 
%    
\paragraph{\textbf{Learning in Relational Lattice}}
We model learning as a meet irreducible deduction transformer \pscmt{???}, called 
{\em abstract unit transformer (AUnit)} \pscmt{why the name? seems confusing as we are talking about learning and not about deduction}.  For an 
abstract lattice $A$ with complementable meet irreducibles 
and a set of conflicting meet irreducibles (countermodels) \pscmt{The term countermodels for $C$ is misleading since $\bigsqcap C$ is a set of countermodels, but the elements of $C$ individually are no countermodels.}
$C \subseteq A$ of $\varphi$ such that $\bigsqcap C$ does not 
satisfy $\varphi$, $AUnit_C: A \rightarrow A$ is defined as follows. 
\pscmt{explain which cases are different in non-relational domains}
\[ AUnit(a) =
 \begin{cases}
 ...
 \end{cases}
\]
For example, let $C = \langle x+z \geq 10,  k \geq 0 \rangle$ \pscmt{why suddenly a different notation for $k$? The variable name $k$ might be confusing because it is usually used for counting or indices} be the conflicting set 
...
consistent} \]   
\pscmt{What is $\mathcal{E}$? What is $m$? What is the notation $\mathcal{E}_1(m,s)\mathcal{E}_2$? What does consistent mean?}
\pscmt{Is there any difference between non-relational and relational?}
