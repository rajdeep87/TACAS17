...
\caption{\label{conflict} Abstract Conflict Graph \pscmt{constants to rhs, please!}}
\end{figure}  
...
However, rule (3) is domain-specific \pscmt{I don't see anyting
  specific here...} and only applies to interval and
octagon\pscmt{relational?}  domain.  For example, let $\conflictset =
\{ x+z \geq 10, w \geq 0 \}$ be a conflicting element \pscmt{better
  name???} that does not satisfy $\formula$ and let $\absval= (x+y
...
meet irreducible of the conflict clause \pscmt{is this $\conflictset$???}
