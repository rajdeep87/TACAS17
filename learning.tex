...
of meet irreducibles $C \subseteq A$ of $\varphi$ \pscmt{???} such that $\bigsqcap C$ does not 
...
domains \pscmt{???}.  For example, let $C = \langle x+z \geq 10,  w \geq 0 \rangle$ 
...
a decision level \pscmt{explained?} that restores the solver to a consistent state 
